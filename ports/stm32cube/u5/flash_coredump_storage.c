...
bool memfault_stm32cubeu5_flash_clear_ecc_error(
  ...
  HAL_FLASH_Unlock();
  {
      ...
      res = HAL_FLASH_Program(
          FLASH_TYPEPROGRAM_QUADWORD, corrupted_flash_address, clear_error);
  }
  HAL_FLASH_Lock();
...
bool memfault_platform_coredump_storage_erase(uint32_t offset, size_t erase_size) {
  ...
  if (erase_begin_addr < FLASH_BASE || erase_begin_addr > stm32_u5_flash_end_addr) {
    return false;
  }
  ...
  if (erase_begin_addr < stm32_u5_flash_bank1_end_addr) {
    const uint32_t bank1_erase_end_addr = MEMFAULT_MIN(stm32_u5_flash_bank1_end_addr, end_addr);
  ...
  const size_t bank2_page_start =
      (erase_begin_addr - stm32_u5_flash_bank1_end_addr) / FLASH_PAGE_SIZE;
